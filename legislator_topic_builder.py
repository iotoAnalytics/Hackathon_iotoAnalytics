...
df = df.drop('', 1)
col_list= list(df)
col_list.remove('goverlytics_id')
df['total'] = df[col_list].sum(axis=1)
rslt_df = df[df['total'] > 0]
topic_lst = ['agriculture',
 'civil_rights',
 'defense',
 'domestic_commerce',
 'edu_cation',
 ...
 'technology',
 'transportation']
df_subset = rslt_df[topic_lst]
df1 = df_subset.idxmax(axis=1)
df_spon = df.merge(df1.rename('top_topic'), left_index=True, right_index=True)
df = df.merge(df_spon, how='left')
df = df.replace(np.nan, 'no bills sponsored', regex=True)
...
for i in range(0,len(df)):
    ...
    row.total = int(df['total'].iloc[i])
    row.top_topic = str(df['top_topic'].iloc[i])

    ca_fed_lst.append(row)

    scraper_utils.write_data(ca_fed_lst, 'ca_fed_legislator_sponsor_topics_total')