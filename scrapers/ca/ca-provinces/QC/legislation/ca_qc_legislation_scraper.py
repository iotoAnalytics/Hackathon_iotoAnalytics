...
def scrape_bill_link(myurl):
    ...
    for pa in possible_actions:
        if any(ext in pa for ext in months_check):
            ...
            try:
                ...
                if "vote : " in event_description.lower():
                    ...
                    yea = int(pour)
                    contre = event_description.lower().split("contre")[
                        1].strip()
                    contre = contre.split(",")[0].strip()
                    contre = contre.replace(":", "").strip()
                    nay = int(contre)
                    abstention = event_description.lower().split("abstention")[
                        1]
            ...
            if "pdf" not in event_description:
                action = {'date': date, 'action_by': 'National Assembly',
        ...
        if "Committee" in pa:
            ...
            # print(com_name)
            committee_info = {
                'chamber': 'National Assembly', 'committee': com_name}
...
if __name__ == '__main__':
    ...
    i = 1
    while failed == 0:
        bill_link = 'http://www.assnat.qc.ca/en/travaux-parlementaires/projets-loi/projet-loi-' + \
            str(i) + '-42-1.html'
        try:
