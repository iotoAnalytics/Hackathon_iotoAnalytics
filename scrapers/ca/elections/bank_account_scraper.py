...
from scraper_utils import BankAccountUtils
...
def get_account_data():
    ...
    for i in items:
        try:
            ...
            pass
        try:
            if 'Total credits to bank account' in line_header.text:
                total_credits = i.find_element_by_tag_name('td').text.replace(',', '')
            if 'Total debits to bank account' in line_header.text:
                total_debits = i.find_element_by_tag_name('td').text.replace(',', '')
            if 'BANK ACCOUNT BALANCE AS OF' in line_header.text:
                total_balance = i.find_element_by_tag_name('td').text.replace(',', '')
            if 'Total amount of outstanding cheques' in line_header.text:
                outstanding_cheques = i.find_element_by_tag_name('td').text.replace(',', '')
            if 'Total amount of outstanding deposits' in line_header.text:
                deposits_in_transit = i.find_element_by_tag_name('td').text.replace(',', '')
            if 'EFFECTIVE BANK ACCOUNT BALANCE' in line_header.text:
                account_balance = i.find_element_by_tag_name('td').text.replace(',', '')
            ...
            try:
                ...
                if 'Total amount of outstanding cheques' in line_header.text:
                    outstanding_cheques = i.find_element_by_tag_name('td').text.replace(',', '')
                if 'Total amount of deposits in transit' in line_header.text:
                    deposits_in_transit = i.find_element_by_tag_name('td').text.replace(',', '')
                if 'EFFECTIVE BANK ACCOUNT BALANCE' in line_header.text:
                    account_balance = i.find_element_by_tag_name('td').text.replace(',', '')
...
def get_row_data(data):
    ...
    row.total_credits = data['total_credits']
    row.total_debits = data['total_debits']
    row.total_balance = data['total_balance']
    row.outstanding_cheques = data['outstanding_cheques']
    row.deposits_in_transit = data['deposits_in_transit']
    row.account_balance = data['account_balance']
    return row
