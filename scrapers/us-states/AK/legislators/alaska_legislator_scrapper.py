...
def find_phone(link):
    ...
    try:
        ...
        lst.append({'office': jlk[1].replace('\r', ''), 'phone': jlk[4].replace('Phone: ', '').strip()})
    except IndexError:
        lst.append({'office': '', 'phone': ''})
    try:
        klk = contact_2[1].text.split('\n')
        lst.append({'office': klk[1].replace('\r', ''), 'phone': klk[4].replace('Phone: ', '').strip()})
    except IndexError:
        lst.append({'office': '', 'phone': ''})
    return lst
...
def find_years(link):
    ...
    for item in x:
        if '/' in item:
            ...
            item = item.split('-')
            lst = lst + [t for t in range(int(item[0]), int(item[1]))] + [int(item[1])]
...
def get_com_info(url):
    ...
    if url_summary is None:
        ...
        for item in url_summary.find_all('li'):
            ...
            com_dict = {'role': dict_items[0], 'committee': dict_items[1] + ' ' + dict_items[2]}
            lst.append(com_dict)
    return lst
...
def find_com(link):
    url_request = request_url.UrlRequest.make_request(link, header)
    url_soup = BeautifulSoup(url_request.content, 'lxml')
    tab_html = url_soup.find('div', {'class': '', 'id': 'tab7_2'}).find_all('li')
    lst = []
    ...
    for item in tab_html:
        ...
        if last_term in item.text:
            com_link = split_url[0] + last_term.replace(re.search('[a-z]{2}', last_term).group(), '') + split_url[1]
            lst = lst + get_com_info(com_link)
    return lst
...
def get_wiki_links(data_dict_item):
    ...
    for item in url_table + url_table2:
        if item.text == data_dict_item['Full Name']:
            data_dict_item['wiki'] = 'https://en.wikipedia.com' + item.find('a').get('href')
...
def scrape_gov(data_dict):
    ...
    if data_dict['Party'] == 'Democrat' or data_dict['Party'] == 'Republican':
