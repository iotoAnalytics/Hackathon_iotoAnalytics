...
def get_urls():
    ...
    chambers = soup.find_all('div', class_='col-sm-2 col-md-2 d-none d-md-block d-lg-block d-xl-block')
    districts = soup.find_all('div', {'class':'col-md-2 col-md-2 d-none d-md-block d-lg-block d-xl-block', 'aria-colindex':4})
...
def scrape_wiki(url, row):
    ...
    try:
        ...
        years_active = list(set(years_active + list(range(int(years_active), datetime.date.today().year + 1))))
...
def scrape(urls):
    ...
    trs = table.find_all('div', class_='col-sm-12 d-sm-block d-md-none d-lg-none d-xl-none')
    ...
    for tr in trs:
        ...
        if label == 'Phone:':
            row.phone_number.append({'district office' : data.replace('(', '').replace(')', '').replace(' ', '-')})
            ...
            years = re.sub('\d\d\d\d-\d\d\d\d', '', data) # We do not look at years ranges, what the years represent is too hard to figure out
            ...
            year_range += list(range(year_range[-1] , datetime.datetime.now().year + 1)) # this has potential for error, I took a guess that the last year continues to current date
