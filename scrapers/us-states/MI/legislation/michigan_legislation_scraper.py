...
def get_bill_info(link):
    ...
    try:
        ...
        sponurls = []
        sponsors_id = []
        sponsorList = page_soup.find("span", {"id": "frg_billstatus_SponsorList"})
        ...
        committees = []
        topics = []
        categorylist = page_soup.find("span", {"id": "frg_billstatus_CategoryList"})
        if categorylist:
        ...
        words_sections = page_soup.find("span", {"id": "frg_billstatus_ObjectSubject"})
        if words_sections:
            bill_summary = words_sections.text

        actions = []
        votes = []
        history_table = page_soup.find("table", {"id": "frg_billstatus_HistoriesGridView"})
                ...
                if i % 3 == 0:
                    ...
                    if "roll call" in description.lower():
                        ...
                        try:
                            ...
                            split_on_yeas = description.lower().split("yeas ")[1]
                            split_on_nays = description.lower().split("nays ")[1]
                            split_on_excused = description.lower().split("excused ")[1]
        ...
        status_table = page_soup.find("table", {"id": "frg_billstatus_DocumentGridTable"})

        bill_text = ""
        doclist = status_table.findAll("a", {"target": "_top"})
        for doc in doclist:
            doc_link = "https://www.legislature.mi.gov" + (doc["href"]).replace("..", "")
            if ".pdf" in doc_link:
                pdf_link = doc_link
        try:
