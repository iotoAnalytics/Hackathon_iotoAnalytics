...
def get_delegate_links(myurl):
    ...
    house_table = page_soup.find("table", {"summary": "Listings of the Virginia House of Delegates"})
    ...
    for office in offices:
        ...
        if "Capitol Office" in office.text or "District Office" in office.text:
            ...
            if "Physical Address" in address_lines[1].text:
                ...
                addr = addr.replace("\n\r\n", "")
                addr = addr.replace(" \r\n", ", ")
                address_info = {'location': 'Physical Address', 'address': addr}
                ...
                addr = address_lines[1].text.strip() + "," + address_lines[2].text.strip()
...
def find_senate_bio_links(myurl):
    ...
    for p in people:
        ...
        phone_number = []
        pn_info = {'office': 'Pocahontas Building', 'number': pocahontas_number}
    ...
    try:
        ...
        for hs in hsections:
            if hs.text == "Session Office":
                    ...
                    try:
                        ...
                        if "Phone" in add_next_line:
                            ...
                            if str(add_next_line).strip() != "":
                                so_address = so_address + " " + str(add_next_line).strip()
                ...
                addr = {'location': 'session office', 'address': so_address.replace("\n", "").strip()}
        ...
        addr = {'location': 'district office', 'address': do_address.replace('\n', '')}
        addresses.append(addr)
...
if __name__ == '__main__':
    ...
    with Pool() as pool:
        rep_bio_data = pool.map(func=scrape_rep_bio_page, iterable=delegatelinks)
    ...
    with Pool() as pool:
        rep_wiki_data = pool.map(func=find_wiki_rep_data, iterable=wiki_del_links)
    rep_wiki_df = pd.DataFrame(rep_wiki_data)
    mergedRepsData = pd.merge(rep_df, rep_wiki_df, how='left', on=["name_first", "name_last"])
    ...
    with Pool() as pool:
        sen_wiki_data = pool.map(func=find_wiki_sen_data, iterable=sen_bios_wiki)
    sen_wiki_df = pd.DataFrame(sen_wiki_data)
    mergedSensData = pd.merge(sen_df, sen_wiki_df, how='left', on=["name_first", "name_last"])
