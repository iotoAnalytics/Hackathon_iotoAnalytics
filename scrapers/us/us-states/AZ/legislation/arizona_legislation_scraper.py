...
def collect_bill_data(myurl):
    ...
    try:
        ...
        element_present = EC.presence_of_element_located(
            (By.CLASS_NAME, 'row content-large'))
        ...
        for event in events:
            try:
                ...
                date = datetime.datetime.strptime(
                    event_date, "%m/%d/%Y").strftime("%Y-%m-%d")
                committee = event_info[1].text
                description = event_info[3].text
                action = {'date': date, 'action_by': chamber,
    ...
    tables = bill_soup.findAll(
        "table", {"class": "table table-bordered table-striped table-bsicondensed"})
    for table in tables:
        ...
        if table_info[2].text != "0":
            try:
                ...
                date = datetime.datetime.strptime(
                    vote_date, "%m/%d/%Y").strftime("%Y-%m-%d")
...
if __name__ == '__main__':
    ...
    bill_df['principal_sponsor_id'] = bill_df['principal_sponsor_id'].replace({
                                                                              np.nan: None})
    bill_df['principal_sponsor_id'] = [
        int(d) if d else d for d in bill_df['principal_sponsor_id']]
    #
