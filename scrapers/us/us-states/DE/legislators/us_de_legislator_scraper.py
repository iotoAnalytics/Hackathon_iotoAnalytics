...
def get_wiki_url(row):
    ...
    wikipage_reps = "https://ballotpedia.org/Delaware_House_of_Representatives"
    wikipage_senate = "https://ballotpedia.org/Delaware_State_Senate"
...
def get_links(url, wiki_list):
    ...
    url_request = UrlRequest.make_request(url, header)
    url_soup = BeautifulSoup(url_request.content, 'lxml')
    district_lst = url_soup.find_all(
        'div', {'class': 'col-xs-24 col-sm-12 col-md-24 col-lg-12 distlinks'})
...
def get_phone(soup, location):
    phone = soup.find('div', {'class': 'info-phone'}
                      ).text.replace('\n', '').split(':')[1]
    ...
    for item in soup.find_all('div', {'class': 'row'}):
        try:
            if 'Committees' in item.find('h3', {'class': 'section-head'}).text:
                for el in item.find_all('li'):
                    com = el.text.split(',')
                    com_dict = {'role': com[1].strip(
                    ), 'committee': com[0].strip()}
    ...
    try:
        if len(is_present.split(',')) == 2:
            ...
            if ('Present' in is_present or 'present' in is_present) and len(years_range) == 1:
                ...
                years_active = [x for x in range(
                    first_year, int(current_year))] + [current_year]
            elif len(years_range) > 1:
                years_active = [x for x in range(int(years_range[0]), int(
                    years_range[-1]))] + [int(years_range[-1])]
            else:
            ...
            if 'Present' in is_present or 'present' in is_present:
                years_active = [x for x in range(
                    years[0], current_year)] + [current_year]
            else:
                years_active = [x for x in range(
    ...
    for item in info_box.find_all('tr'):
        if 'Alma mater' in item.text or 'Education' in item.text:
            ...
            if ')' and '(' in item.text:
                ...
                edu_text = item.text.replace(
                    'Alma mater', '').replace('Education', '')
                edu_list = [x.strip() for x in edu_text.split(')') if x]
                for el in edu_list:
                    split_el = el.split('(')
                    edu_dict = {
                        'level': split_el[1], 'field': '', 'school': split_el[0].strip()}
                    edu.append(edu_dict)
            else:
                unis = [x.get('title')
                        for x in item.find_all('a') if educated(x.text)]
                for el in unis:
...
def scrape(legis_dict):
    ...
    row.email = url_soup.find_all(
        'div', {'class': 'info-group'})[1].find('a').text
