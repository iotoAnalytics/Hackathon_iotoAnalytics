...
def get_sponsor(soup):
    try:
        sponsor_text = soup.find(
            'span', {'id': 'ctl00_ContentPlaceHolderCol1_ListView1_ctrl0_introducerLabel'}).text
        sponsor_lst = sponsor_text.replace(
            '(Introduced by request of another party)', '').strip().split(',')
...
def get_actions(soup):
    try:
        table = soup.find(
            'table', {'id': 'ctl00_ContentPlaceHolderCol1_GridViewStatus'})
    ...
    try:
        ...
        for sponsor in sponsors:
            if re.match('[A-Z]\..*', sponsor):
                name_last = sponsor.replace(
                    re.match('[A-Z]\.', sponsor).group(), '').strip().title()
