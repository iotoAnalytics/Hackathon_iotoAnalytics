...
def get_name(soup):
    name = soup.find(
        'span', {'id': 'ctl00_ContentPlaceHolderCol1_LabelLegname'}).text
...
def get_role(soup):
    role = soup.find(
        'span', {'id': 'ctl00_ContentPlaceHolderCol1_LabelSenRep'}).text
    return role


def get_district(soup):
    district = soup.find(
        'span', {'id': 'ctl00_ContentPlaceHolderCol1_LabelDistrict'}).text
    return district


def get_phone(soup):
    phone = soup.find(
        'span', {'id': 'ctl00_ContentPlaceHolderCol1_LabelPhone'}).text
    box_text = soup.find('div', {
                         'class': 'roundedrect gradientgray shadow'}).text.replace('\r', '').split('\n')
...
def get_rep_wikilink(soup):
    ...
    rep_table = wiki_soup.find(
        'table', {'class': 'sortable wikitable'}).find_all('a')
    url = None
...
def get_sen_wikilink(soup):
    ...
    wiki_soup = BeautifulSoup(wiki_req.content, 'lxml')
    sen_table = wiki_soup.find(
        'table', {'class': 'wikitable sortable'}).find_all('a')
    url = None
    ...
    for item in test_lst:
        try:
            if 'Education' in item.text or 'Alma mater' in item.text:
                temp = item.text.replace('Education', '').replace(
                    'Alma mater', '').split(')')
                for elem in temp:
                    if elem != '':
                        uni_info = elem.split('(')
                        dict_list.append(
                            {'level': uni_info[1].strip(), 'field': '', 'school': uni_info[0].strip()})
        except:
            pass
        ...
        try:
            for item in rep_table.find_all('td'):
                if 'office' in item.text:
                    ...
                    if len(temp) == 2:
                        temp_lst = [x for x in range(
                            int(sorted(temp)[0]), int(sorted(temp)[1]) + 1)]
...
def get_email(soup):
    email = soup.find(
        'a', {'id': 'ctl00_ContentPlaceHolderCol1_HyperLinkEmail'}).text
    return email
...
def get_com(soup, name):
    ...
    try:
        com_soup = soup.find(
            'table', {'id': 'ctl00_ContentPlaceHolderCol1_GridViewMemberof'}).find_all('tr')
        for item in com_soup:
            ...
            vice_chair = link_soup.find(
                'a', {'id': 'ctl00_ContentPlaceHolderCol1_HyperLinkcvChair'}).text.strip()
            if name.strip() == chair:
                role = 'Chair'
...
def get_dicts(url):
    ...
    for item in url_tr:
        try:
            ...
            areas_lst = [x.strip().replace('\x80\x98', '').replace('â', '')
                         for x in areas]
...
def get_wiki_url(row):
    ...
    wikipage_reps = "https://ballotpedia.org/Hawaii_House_of_Representatives"
    wikipage_senate = "https://ballotpedia.org/Hawaii_State_Senate"
