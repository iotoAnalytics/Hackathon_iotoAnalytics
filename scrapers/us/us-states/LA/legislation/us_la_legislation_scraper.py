...
def scrape(url):
    ...
    chamber_origin = get_chamber_origin(bill_name, row)
    ...
    get_votes(chamber_origin, driver, row)
...
def get_chamber_origin(bill_name, row):
    if "HB" in bill_name:
...
def get_votes(chamber_origin, driver, row):
    # vote
    item_bar_text = []
    ...
    if "Votes" in item_bar_text:
        ...
        for a in votes_text_locations:
            ...
            votes += votes_reformat(votes, yeas_string, "yea")
            nays_string = re.search('NAYS(.*?)Total', votes_text).group(1)
            votes += votes_reformat(votes, nays_string, "nay")
            absent_string = re.search('ABSENT(.*?)Total', votes_text).group(1)
            votes += votes_reformat(votes, absent_string, "absent")
...
def votes_reformat(votes, name_string, votetext):
    name_string = replace_speaker_name(name_string)
    name_string = name_string.replace(", ", ",")
    yea_voters = name_string.split(" ")
    for name in yea_voters:
        if name != "":
            if "," not in name:
                voter_id = scraper_utils.get_legislator_id(name_last=name)
                ...
                votes.append({"goverlytics_id": voter_id, "legislator": name, "votetext": "yea"})
