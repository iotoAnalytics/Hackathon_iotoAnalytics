...
def collect_vote_info(link):
    ...
    for name in ayes:
        ...
        v = {"legislator": legislator,
             "goverlytics_id": goverlytics_id, "vote": vote}
        votes.append(v)
...
def collect_bill_details(bill_url):
    ...
    # find bill title
    titlemain = page_soup.find(
        "main", {"class": "col-12 body-content ncga-container-gutters"})
            ...
            for lefttag in left_column_tags:
                if "(Primary)" in lefttag.text:
                    ...
                    if "." in principal_sponsor:
                        principal_sponsor = principal_sponsor.split(".")[
                            1].strip()
