# TODO - Refactor
# TODO - What is FAIL condition for passing a legislation
...
class OKLegislationUtils:
    ...
    def get_session(session_code):
        if session_code == self.REGULAR_SESSION_2021:
            return '2021 Regular Session'

    def get_chamber(chamber_code):
        if chamber_code == self.SENATE:
            return 'Senate'
        elif chamber_code == self.HOUSE_CHAMBHOUSEER:
            return 'House'

    def get_bill_type(bill_code):
        if bill_code == self.SENATE_RESOLUTION:
            return 'Resolution'

    def get_status(status_code):
        if status_code == self.ENROLLED:
            return 'Enrolled, filed with Secretary of State'

    def get_sponsor_role_from_abbr(sponsor_role_abbr):
        if sponsor_role_abbr == self.SENATE:
            return 'Senator'
        elif sponsor_role_abbr == self.HOUSE:
            return 'Representative'

    def get_sponsor_role_from_url(sponsor_url):
        if self.SENATE_DOMAIN in sponsor_url:
            return 'Senator'
        elif self.HOUSE_DOMAIN in sponsor_url:
            return 'Representative'


class OKLegislationParser:
    # Main function to get votes
    ...
    def __divide_into_pages(self, text):
        ...
        for idx, line in enumerate(text):
            if 'Top_of_Page' in line:
                ...
                first_line_of_page = idx + 1 # Don't include the Top_of_Page
        else:
            # Add last page
    ...
    def __parse_page(self, page):
        ...
        if 'OKLAHOMA HOUSE OF REPRESENTATIVES' in title:
            votes = OKHouseParser().parse_votes(page, first_line_of_sections)

        elif 'Committee' in title:
            votes = OKHouseCommitteeParser().parse_votes(page, first_line_of_sections)

        elif 'THE OKLAHOMA STATE SENATE' in title and 'Legislature' not in line_after_title:
            print('Scraping with senate parser')
            votes = OKSenateParser().parse_votes(page, first_line_of_sections)

        elif 'THE OKLAHOMA STATE SENATE' in title:
            votes = OKSenateCommitteeParser().parse_votes(page, first_line_of_sections)
    ...
    def _get_description(self, page, idx, is_bad_break=False):
        ...
        # Remove extra words (PASSED)
        description = description.replace('PASSED', '').strip()

        return description

    # def _get_bill_description(self, page, idx, is_bad_break=False):
    #     # Gets description of the bill.
    #     # Usually formatted similar to the following:
    #     #    SB821               Patient's Right to Pharmacy Choice Act; modifying
    #     #    McEntire            certain contract restrictions; modifying
    ...
    #     while 'PASS' not in line and not line.isspace():
    #         if line_count < 2:
    #             formatted_line = re.sub('[a-zA-Z0-9]+\:?(\,?\s[a-zA-Z0-9]+)*\s{2}', '', line, 1) # Remove left column
    #             formatted_line = formatted_line.strip() + padding
    #             description += formatted_line
    #             line_count += 1
    #         else:
    #             description += line.strip() + padding
    ...
    def _get_voters(self, page, idx, sections_dict, sections_dict_key):
        ...
        while (line := page[idx]).isspace() != True:
            voters.append(line + ' ') # Add extra space for padding
            idx += 1
    ...
    def _format_voters_list(self, voters, voter_role, vote_type):
        # Format voters list for the votes field of votes
        formatted_voters = []
        for voter in voters:
    ...
    def _get_voter_data(self, voter, role, vote):
        ...
        t = re.search('\([a-zA-Z\.]+\)|\,\s[a-zA-Z\.]+', voter) # Dossett (J.A.) or Dossett, J.A

        if t != None:
            first_initial = re.sub('[\(\)]|\,\s', '', t.group(0))
...
class OKHouseParser(OKLegislationParser):
    CHAMBER = 'House'
    ROLE = 'Representative'
    ...
    def parse_votes(self, page, first_line_of_sections):
        ...
        # print(f"Y: {vote_data['yea']}, N: {vote_data['nay']}, NV: {vote_data['nv']}, A: {vote_data['absent']}")
...
class OKHouseCommitteeParser(OKLegislationParser):
    CHAMBER = 'House'
    ROLE = 'Representative'
...
class OKSenateParser(OKLegislationParser):
    CHAMBER = 'Senate'
    ROLE = 'Senator'
    ...
    def _divide_into_sections(self, first_line_of_sections):
        ...
        for i, line in enumerate(first_line_of_sections):
            if 'THE OKLAHOMA STATE SENATE' in line:
                sections_dict['title'] = line
                sections_dict['description'] = first_line_of_sections[i + 2]
                # TODO - Senate Bills doesn't have a status
                # sections_dict['status'] = first_line_of_sections[i + 2]
                sections_dict['vote_outcome'] = first_line_of_sections[i + 3]
...
class OKSenateCommitteeParser(OKLegislationParser):
    CHAMBER = 'Senate'
    ROLE = 'Senator'
